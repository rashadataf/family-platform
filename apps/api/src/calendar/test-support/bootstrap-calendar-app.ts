...
import { CALENDAR_CLOCK } from '../calendar.tokens.js';
...
/** The "now" every calendar suite runs at unless it moves the clock: the day spec 009 was implemented. */
export const CALENDAR_TEST_NOW = '2026-09-15T10:00:00Z';

/**
 * `bootstrapTestApp`'s real app graph, with Calendar's clock pinned.
 *
 * Materialisation windows are measured from "now", so a recurrence test run on
 * a wall clock would pass or fail depending on the day it ran — the kind of
 * test that is green for a year and red on a boundary nobody reproduces. Only
 * `CALENDAR_CLOCK` is replaced; sessions and families keep the system clock,
 * because nothing they assert depends on a date.
 */
export async function bootstrapCalendarApp(
  ...
  const clock = fixedClock(options.now ?? CALENDAR_TEST_NOW);

  const moduleRef = await Test.createTestingModule({ imports: [buildAppModule(env)] })
    .overrideProvider(MAILER)
    .useValue(mailer)
    .overrideProvider(CALENDAR_CLOCK)
    .useValue(clock)
    .compile();
