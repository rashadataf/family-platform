...
**Development Workflow gate table**: "Infrastructure validation and preview" is satisfied by the new `infra-preview` CI job (`pulumi preview` on every pull request, contracts/cli-and-config.md). "Security scan: dependency vulnerabilities and secret scanning" was an open gap when this plan was first written; [spec 005](../005-merge-gate-enforcement/spec.md) closed it (the `security` job, plus a `trivy` step in `image`) before this feature reached implementation. `infra-deploy` MUST wait on that job along with every other blocking check spec 005 added — see the corrected CI trigger contract in contracts/cli-and-config.md.
