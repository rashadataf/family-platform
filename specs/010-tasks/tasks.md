...
- [ ] T001 [P] Create `packages/core/src/tasks/index.ts` (empty barrel) and re-export it from
      `packages/core/src/index.ts` as `export * as tasks from './tasks/index.js';` alongside
      `calendar`, `family`, `identity` and `compliance`.
- [ ] T002 [P] Add a `tasks-repositories-are-private` rule to `.dependency-cruiser.cjs`, forbidding
...
- [ ] T003 [P] Add `TaskId` and `TaskSeriesId` with `asTaskId` / `asTaskSeriesId` to
      `packages/kernel/src/branded-id.ts`, exported from `packages/kernel/src/index.ts`.
- [ ] T004 [P] Add task error kinds to the `DomainError` union in `packages/kernel/src/errors.ts`:
...
- [ ] T005 Add the `Task` and `TaskAssignment` models and the `TaskStatus`, `TaskPriority`,
...
- [ ] T006 Generate `packages/persistence/prisma/migrations/<timestamp>_tasks/migration.sql`, then
...
- [ ] T007 In the same migration, hand-write: the partial unique index
...
- [ ] T008 [P] Declare `TasksUnitOfWork` and `TasksUnitOfWorkPort` in
...
- [ ] T009 Implement `withTasksFamilyContext` in `packages/persistence/src/tasks-context.ts` (one
      ...
      `calendar-context.ts`.
- [ ] T010 [P] Integration test `packages/persistence/src/tasks-context.integration.spec.ts`: with no
...
- [ ] T011 [P] Integration test in `tasks-context.integration.spec.ts`: `set_config(…, true)` does not
      survive its transaction on a pooled connection.

### Ports and events

- [ ] T012 [P] Declare `TaskRepository` and `TaskAssignmentRepository` in
...
- [ ] T013 [P] Declare `TasksReadPort` in `packages/core/src/tasks/application/ports/tasks-read.port.ts`
      (open list, due range, history, single read, each taking a reader) for the later dashboard and
      AI read paths (§7.1).
- [ ] T014 [P] Declare `ErasurePort` (`eraseForFamily`, `eraseForMember`) in
      `packages/core/src/tasks/application/ports/erasure.port.ts`, mirroring Calendar's.
- [ ] T015 [P] Implement the six versioned event builders in `packages/core/src/tasks/domain/events.ts`
...
- [ ] T016 [P] Add `packages/testing/src/tasks-factories.ts` (an undated task, a date-only task, a
...
- [ ] T017 [P] Create `apps/api/src/tasks/test-support/household.ts` and
...
- [ ] T018 Create `apps/api/src/tasks/tasks.module.ts` and `tasks.tokens.ts`: clock, tasks unit of
...
- [ ] T019 Create `packages/contracts/src/v1/tasks.contract.ts` with the router skeleton and shared
...
- [ ] T020 [P] [US1] Unit test `packages/core/src/tasks/domain/due.spec.ts`: `date` due moment is
      ...
      `UnknownTimeZone`.
- [ ] T021 [P] [US1] Unit test `packages/core/src/tasks/domain/task.aggregate.spec.ts` (creation
      only): title 1–200 chars, notes ≤ 4,000, default priority `normal`, starts `open` at version 1,
      `due_kind: none` has no zone.
- [ ] T022 [P] [US1] Integration test `apps/api/src/tasks/create-task.integration.spec.ts`:
...
- [ ] T023 [P] [US1] Integration test `apps/api/src/tasks/list-open-tasks.integration.spec.ts`:
...
- [ ] T024 [P] [US1] Integration test `apps/api/src/tasks/capability.integration.spec.ts`: Viv
      (viewer) can `GET` but `POST …/tasks` is `403 task/capability_required`; Alan (extended)
      succeeds.
- [ ] T025 [P] [US1] Integration test `apps/api/src/tasks/cross-family-access.integration.spec.ts`
...
- [ ] T026 [US1] Implement `packages/core/src/tasks/domain/due.ts`: the `Due` union and
      `dueMomentOf(due)`, pure, using only `@fp/kernel/recurrence`'s `isValidTimeZone`,
      `parseLocalDate`, `addDays` and `localToInstant`.
- [ ] T027 [US1] Implement `packages/core/src/tasks/domain/task.aggregate.ts` with creation, field
...
- [ ] T028 [P] [US1] Implement `isOverdue(task, now)` in `packages/core/src/tasks/domain/overdue.ts`.
- [ ] T029 [US1] Implement `packages/persistence/src/repositories/tasks/task.repository.ts`: `insert`,
...
- [ ] T030 [US1] Implement `createTask` in
...
- [ ] T031 [P] [US1] Implement `listOpenTasks` in
...
- [ ] T032 [P] [US1] Implement `getTask` in `packages/core/src/tasks/application/queries/get-task.query.ts`.
- [ ] T033 [US1] Add `POST …/tasks`, `GET …/tasks` and `GET …/tasks/:taskId` to
      `packages/contracts/src/v1/tasks.contract.ts`.
- [ ] T034 [US1] Implement `apps/api/src/tasks/tasks.controller.ts` binding those routes with
...
- [ ] T035 [P] [US2] Unit test `packages/core/src/tasks/domain/task-assignment.spec.ts`: an
...
- [ ] T036 [P] [US2] Integration test `apps/api/src/tasks/assignees.integration.spec.ts`:
...
- [ ] T037 [P] [US2] Integration test `apps/api/src/tasks/child-visibility.integration.spec.ts`,
...
- [ ] T038 [P] [US2] Integration test `apps/api/src/tasks/child-audit.integration.spec.ts`: a
...
- [ ] T039 [US2] Implement `packages/core/src/tasks/domain/task-assignment.ts`.
- [ ] T040 [US2] Implement `packages/persistence/src/repositories/tasks/task-assignment.repository.ts`
...
- [ ] T041 [US2] Implement `packages/core/src/tasks/application/visibility.ts`: `resolveVisibility`
...
- [ ] T042 [US2] Apply the visible-set filter **in SQL** in `task.repository.ts`'s `listOpen`:
...
- [ ] T043 [US2] Extend `getTask` and `listOpenTasks` to resolve visibility first, return `NotFound`
      for a hidden task, and audit guarded-child reads in the same transaction.
- [ ] T044 [US2] Implement `assign-task.command.ts` and `unassign-task.command.ts` in
...
- [ ] T045 [US2] Add `PUT` and `DELETE …/tasks/:taskId/assignees/:memberId` and `assigneeIds` on
...
- [ ] T046 [P] [US3] Unit test in `packages/core/src/tasks/domain/task.aggregate.spec.ts`: an
...
- [ ] T047 [P] [US3] Integration test `apps/api/src/tasks/lifecycle.integration.spec.ts`: complete
...
- [ ] T048 [P] [US3] Integration test `apps/api/src/tasks/update-task.integration.spec.ts`: `PATCH`
...
- [ ] T049 [P] [US3] Integration test `apps/api/src/tasks/concurrency.integration.spec.ts`: two
...
- [ ] T050 [P] [US3] Integration test `apps/api/src/tasks/idempotency.integration.spec.ts`: every
...
- [ ] T051 [US3] Add `complete(actor, now)`, `reopen()`, `cancel(actor, now, scope)` and
      `update(changes)` to `task.aggregate.ts`, with exhaustive switch handling over the status union.
- [ ] T052 [US3] Add `findByIdForUpdate` (raw `SELECT … FOR UPDATE` in the persistence raw-query
...
- [ ] T053 [US3] Implement `complete-task.command.ts`, `reopen-task.command.ts` and
...
- [ ] T054 [US3] Implement `update-task.command.ts` with the same ordering, recomputing `due_at` via
      `due.ts`.
- [ ] T055 [P] [US3] Implement `listClosed` in `task.repository.ts` (history index, visibility filter in
      SQL) and `list-task-history.query.ts` in `packages/core/src/tasks/application/queries/`.
- [ ] T056 [US3] Add `PATCH …/tasks/:taskId`, `POST …/complete`, `…/reopen`, `…/cancel` and
...
- [ ] T057 [P] [US4] Unit test `packages/kernel/src/recurrence/next-occurrence.spec.ts`: weekly across
...
- [ ] T058 [P] [US4] Unit test `packages/core/src/tasks/domain/successor.spec.ts`:
...
- [ ] T059 [P] [US4] Integration test `apps/api/src/tasks/recurrence.integration.spec.ts`: create with
...
- [ ] T060 [P] [US4] Integration test `apps/api/src/tasks/series-invariants.integration.spec.ts`:
...
- [ ] T061 [P] [US4] Integration test `apps/api/src/tasks/assignee-carry-forward.integration.spec.ts`:
...
- [ ] T062 [US4] Implement `nextOccurrenceAfter` in `packages/kernel/src/recurrence/next-occurrence.ts`
...
- [ ] T063 [US4] Implement `packages/core/src/tasks/domain/successor.ts`: pure
      `successorOf(head, closedAt)` returning the new task's props or null, per
      [data-model.md](data-model.md)'s successor rule.
- [ ] T064 [US4] Extend `task.aggregate.ts` with `recurrenceRule` (a parsed kernel `RecurrenceRule`),
...
- [ ] T065 [US4] In `task.repository.ts`, add `handOverHead(predecessor, successor)`: clear the
...
- [ ] T066 [US4] Extend `complete-task.command.ts` and `cancel-task.command.ts`: when the task is head
      ...
      successor.
- [ ] T067 [US4] Extend `create-task.command.ts` and `update-task.command.ts` for `recurrenceRule`
      (parse via the kernel, mapping to `recurrence_invalid` / `recurrence_unsupported`).
- [ ] T068 [US4] Add `recurrenceRule` on create and `PATCH`, `seriesId` / `isSeriesHead` /
...
- [ ] T069 [P] [US5] Unit test `packages/core/src/tasks/domain/overdue.spec.ts`: `needsOverdueReport`
...
- [ ] T071 [US5] Implement `needsOverdueReport(task, now)` in `packages/core/src/tasks/domain/overdue.ts`.
- [ ] T072 [US5] Implement `report-overdue.command.ts` in
...
- [ ] T073 [US5] Implement `packages/persistence/src/repositories/tasks/overdue-sweep.ts`:
...
- [ ] T074 [US5] Implement `apps/worker/src/sweeps/report-overdue-tasks.sweep.ts` in the shape of
      ...
      only.
- [ ] T075 [US5] Call the sweep from `apps/worker/src/sweep-retention.ts` and add spec 010 to its doc
...
- [ ] T076 [P] Unit test `apps/worker/src/scheduler/scheduler.spec.ts` with fake timers and an
...
- [ ] T077 [P] Unit test `apps/worker/src/config/worker-env.spec.ts`: each
...
- [ ] T078 [P] Integration test `apps/worker/src/scheduler/scheduled-sweeps.integration.spec.ts`
...
- [ ] T079 [P] Unit test in `infrastructure/src/deploy.spec.ts`: the deploy script's `up -d` names
...
- [ ] T080 Create `apps/worker/src/sweeps/registry.ts`: one exported list of
...
- [ ] T081 Refactor `apps/worker/src/sweep-retention.ts` to iterate `registry.ts` in order, keeping
      ...
      unmodified.
- [ ] T082 [P] Implement `apps/worker/src/config/worker-env.ts`: a Zod schema over `process.env` for
      the seven cadences and `WORKER_HEARTBEAT_PATH`, parsed once at boot, failing the process on
      invalid input (Principle II).
- [ ] T083 Implement `apps/worker/src/scheduler/scheduler.ts` with no new dependency, per
...
- [ ] T084 Update `apps/worker/src/main.ts`: parse `worker-env.ts`, build the scheduler from
...
- [ ] T085 Add a `HEALTHCHECK` to the `runtime` stage of `apps/worker/Dockerfile` that fails when
...
- [ ] T086 In `infrastructure/src/image.ts`, build the worker's `runtime` target as
...
- [ ] T087 In `infrastructure/src/transfer.ts`, transfer the worker tarball with its digest dependency.
...
- [ ] T088 Add a `worker` override to `docker-compose.staging.yml`: `image: fp-worker:staging-runtime`,
...
- [ ] T089 [P] Update `docs/staging-environment.md` (the worker is deployed; reading
...
- [ ] T090 Implement `eraseTasksForFamily` and `eraseTasksForMember` in
...
- [ ] T091 [P] Add `apps/api/src/tasks/no-personal-data-in-telemetry.integration.spec.ts`, copying
...
- [ ] T092 [P] Add `packages/core/src/tasks/reads-family-only-through-published-ports.spec.ts`, copying
...
- [ ] T093 [P] Extend `packages/kernel/src/recurrence/purity.spec.ts` so its import and determinism
      assertions cover `next-occurrence.ts`, and confirm `packages/kernel/package.json` gained no
      dependency.
- [ ] T094 [P] Confirm `apps/api/src/family/checks-capabilities-not-roles.spec.ts` scans
      `apps/api/src/tasks/` and `packages/core/src/tasks/`. If its globs are Family- or Calendar-only,
      widen them.
- [ ] T095 Complete `apps/api/src/tasks/cross-family-access.integration.spec.ts` with all ten routes,
...
- [ ] T096 [P] Emit the observability signals from [contracts/tasks-api.md](contracts/tasks-api.md)
      ...
      convention.
- [ ] T097 [P] Apply rate limits in `apps/api/src/tasks/tasks.module.ts` via `PerUserThrottlerGuard`:
      120 per user per minute on both lists, and 120 per user per hour on `POST …/tasks`.
- [ ] T098 [P] Update `ARCHITECTURE.md` §5.4's **Publishes** line to add `TaskUpdated` and
