...
- [X] T040 **Requires the user.** Reconcile branch protection once every new job has reported green at least once (FR-029), using the `gh api -X PATCH .../branches/main/protection/required_status_checks` command in [contracts/gates-and-config.md §4](contracts/gates-and-config.md) with the ten-check target list. **Adding a check before it has ever reported blocks every merge forever on a check that never arrives** — this is the reason for the ordering, not caution for its own sake
- [X] T041 Done, read side by side, and the answer is **seven of nine, not nine of nine**. Written into [contracts/gates-and-config.md §4a](contracts/gates-and-config.md). The two rows with no check are *Contract and API tests* (there is no product API, no `packages/contracts`, no authorization to assert) and *Infrastructure validation* (no infrastructure-as-code; spec 003 parked). Both close with a later feature. Adding a check for either now would mean an empty suite reporting green, which is worse than an acknowledged gap because it makes the table look finished. SC-010 remains open pending T040
- [X] T042 Done on its own branch `005a-constitution-security-todo`, branched from `main` and **not** included in this feature's PR — Governance forbids bundling an amendment with a feature change. It must merge **after** spec 005: striking the TODO before the tooling exists would leave the record claiming a gate that does not. No version bump — the edit is to the sync-impact report, not normative text
- [X] T043 Re-read all four config files against the contract. Four rows of the contract had drifted from what was actually built and are corrected: the `test` job's command, the two extra `verify-env` steps, the `.gitleaks.toml` custom rule, the third Dependabot ecosystem, who enforces the suppression expiry, and `globalSetup` vs `setupFiles`. Job names all match and none was renamed
- [X] T044 Opened as [issue #11](https://github.com/rashadataf/family-platform/issues/11)
